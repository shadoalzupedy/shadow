...
if matches[1]:lower() == 'بره' then
	...
	if matches[1]:lower() == 'حضر عام' then-- /ban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      if is_admin1(msg) then
 ...
 if matches[1]:lower() == 'الغاء العام' then-- /ban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      if is_admin1(msg) then
...
if matches[1]:lower() == 'الغاء الحضر' then-- /ban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      if is_admin1(msg) then
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        unbanall_user(matches[2])
		send_large_msg(receiver, 'العضو الحيوان 😡  ['..matches[2]..'] تم طرده ☺️❤️ ')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'unban',
  ...
  if matches[1]:lower() == "المطرودين" then -- Global ban list
    return banall_list()
  end
end
